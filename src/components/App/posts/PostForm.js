  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        showToast({ title: 'Error!', content: errorData.message || 'Failed to save post.', cssClass: 'e-toast-danger', icon: 'e-error toast-icons' });
        setIsSubmitting(false);
        return;
      }

      showToast({ title: 'Success!', content: 'Post saved successfully.', cssClass: 'e-toast-success', icon: 'e-success toast-icons' });
      navigate('/app/posts/');
    } catch (error) {
