  ...
  const archiveBanner = async (id, is_archived) => {
    ...
    if (confirmArchive) {
      try {
        ...
        const url = `${BASE_URL}/crm/admin-api/banners`; // Adjust URL path as needed
        ...
        // Make the fetch request to update the banner
        ...
        if (response.ok) {
          ...
          navigate(is_archived ? "/app/banners/archived" : "/app/banners/");  // Redirect based on the archive status
        } else {
  ...
  const onDragEnd = async (result) => { 
      ...
      try {
        ...
        if (response.ok) {
          ...
          showToast({ title: 'Success!', content: 'Banner order has been updated.', cssClass: 'e-toast-success', icon: 'e-success toast-icons' });
        } else {
          // If the server responds with an error, inform the user and do not update local state
          const errorData = await response.json();
          showToast({ title: 'Error!', content: errorData.message || 'Failed to update banner order.', cssClass: 'e-toast-danger', icon: 'e-error toast-icons' });
        }
      } catch (error) {
        // Handle any errors that occurred during fetch
        showToast({ title: 'Error!', content: 'Failed to communicate with server.', cssClass: 'e-toast-danger', icon: 'e-error toast-icons' });
      ...
      <Droppable droppableId="banners">
          {(provided, snapshot) => (
            ...
            {currBanners.map((banner, index) => (
              ...
              >
                <div to={`/app/banners/${banner.id}`} onClick={()=>selectHandler(banner)} className="flex items-center hover:bg-gray-100 w-full text-decoration-none">
